/* 
A string is considered to be in title case if each word in the string is either (a) capitalised (that is, only the 
first letter of the word is in upper case) or (b) considered to be an exception and put entirely into lower case 
unless it is the first word, which is always capitalised.
Write a function that will convert a string into title case, given an optional list of exceptions (minor words). 
The list of minor words will be given as a string with each word separated by a space. Your function should ignore
the case of the minor words string -- it should behave in the same way even if the case of the minor word string is changed.


titleCase('a clash of KINGS', 'a an the of') // should return: 'A Clash of Kings'
titleCase('THE WIND IN THE WILLOWS', 'The In') // should return: 'The Wind in the Willows'
titleCase('the quick brown fox') // should return: 'The Quick Brown Fox'
*/

console.log(titleCase('a clash of KINGS', 'a an the of'));
console.log(titleCase('THE WIND IN THE WILLOWS', 'The In'));
console.log(titleCase('the quick brown fox', 'the'));
console.log(titleCase('the QUICK bRoWn fOX', 'xyz fox quick the'));
console.log(titleCase('the QUICK f bRoWn  g fOX '));

function titleCase(title, minorWords) {
  if (!title) return '';

  title = title.replace(/\s+/g, ' ').trim();  // убрать все лишние пробелы.

  minorWords = typeof minorWords === 'string' ? minorWords.toLowerCase().split(' ') : null;

  return title.split(' ')
    .map((word, index, arr) => {
      if (arr.length === 1 || minorWords === null) {
        return `${word[0].toUpperCase()}${word.slice(1).toLowerCase()}`;
      }
      else if (minorWords !== null
        && (!(minorWords.includes(word.toLowerCase())) || index === 0)) {
        return `${word[0].toUpperCase()}${word.slice(1).toLowerCase()}`;
      }
      else {
        return word.toLowerCase();
      }
    })
    .join(' ');
}

